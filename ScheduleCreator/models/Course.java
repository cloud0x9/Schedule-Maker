...
public class Course {
    ...
    private void loadSectionsFromFile(String _semester) {
        ...
        Pattern p = Pattern.compile(".*([0-9]{5}).*- ([0-9]{2})\\s*(\\S* [ap]m - \\S* [ap]m)\\s*(\\S*)(.*)=(..(([A-z]*)( *))*)");
        ...
        Pattern p3 = Pattern.compile(".*([0-9]{5}).*- ([0-9]{2})\\s*(\\S* [ap]m - \\S* [ap]m)\\s*(\\S*)(.*)=(\\s+([A-z]+\\s+)+)(\\S* [ap]m - \\S* [ap]m).([A-z])");
        ...
        for (String sectionString : sectionStrings) {
            ...
            if ((m.matches())) {
                ...
                SecondDaysAndTimes = m3.group(9).trim() + " " + m3.group(8).trim();
